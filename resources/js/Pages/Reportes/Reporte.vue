     ...
     <section class="container pt-3">
          <div class="row">
            ...
            <div class="col-md-8 offset-lg-1 pb-5 mb-2 mb-lg-4 pt-md-5 mt-n3 mt-md-0">
              ...
              <jet-label class="label form-label fs-base" value="* Tipo de reporte" />
              <select class="form-select form-select-lg"  id="tipo" v-model="form.tipo" autofocus required>
                    <option disabled value="">Selecciona tipo</option>
                    <option value="1">Proyectos por temática</option>
                    <option value="2">Proyectos por institución</option>
                    <option value="3">Proyecto por usuario</option>
            </select>
            </div>
            <div class="col-sm-6 mb-4">
            <jet-label v-if="form.tipo != '' " class="label form-label fs-base" value="* Por institución " />
            <select v-if="form.tipo != '' "  v-model="form.tipo2" class="form-select form-select-lg"  id="tipo" autofocus required>
                    <option disabled value="">Seleccione un elemento </option>
                    <option value="">Todas las instituciones</option>
                    <option v-for="item in instituto" v-bind:value="item.id" v-bind:key="item.id">{{ item.name }}</option>
            </select>
            </div>
            <button @click="guardar" class="button ml-4 text-xl" :class="{ '': form.processing }"
                :disabled="form.processing">
                <i class="bi bi-file-bar-graph"></i> <span class="animate-spin mr-1" v-show="form.processing">&#9696;</span> Generar reporte
            </button>
            </form>
          </div>
        </div>

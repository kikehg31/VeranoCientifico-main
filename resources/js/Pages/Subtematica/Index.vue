...
<div class="contenido">
    <div class="forma1">
      <h2 class="h4 font-weight-bold">
        <i class='boxi bx bx-book' style='color:#030664' ></i> Subtemáticas Registradas
      </h2><hr><br>
        ...
        <div class="card-body border-right border-bottom p-3 h-100">
          ...
          <div class="table-responsive">
            <table class="table table-bordered">
              <thead>
                <tr>
                  <th width="6%" class="col">No.</th>
                  <th width="25%" class="col">Nombre</th>
                  <th width="15%" class="col">Temática a la que pertenece</th>
                  <th width="9%" class="col">Editar</th>
                  <th width="9%" class="col">Eliminar</th>
                </tr>
              </thead>
              <tbody v-for="item in subtematicas.data" :key="item.id">
                <tr>
                  <td>{{ item.id  }}</td>
                  <td>{{ item.name }}</td>
                  <td>{{ item.tematica.name }}</td>
          ...
          <pagination  :links="subtematicas.links" :total="subtematicas.total" class="pagination"/>
        </div>
      </div>
    </div>
  </div>
    ...
    export default {
        ...
        setup(props) {
          const form = useForm({ ...props.subtematicas });
            const thereAreResults = computed(() => props.subtematicas.total > 0);
            const state = reactive({
                filters: {
                    page: props.subtematicas.current_page,
                    search: props.search,
            ...
            const eliminar = (idel) => {
              ...
              if (res.isConfirmed) {
                form.delete(route("subtematicas.destroy", idel));
              }
            });
          };
